...
const { getPendingVerifications, approveWorker, rejectWorker } = require('../controllers/admin.controller');
...
describe('getPendingVerifications', () => {
  ...
  it('returns pending workers with user info', async () => {
    const workers = [
      { _id: 'w1', user: { name: 'Ali', email: 'ali@test.com' }, services: ['Plumbing'], idDocument: '' },
    ];
    Worker.find.mockReturnValue({
      select: jest.fn().mockReturnThis(),
...
describe('approveWorker', () => {
  ...
  it('approves worker and sets isVerified', async () => {
    const worker = { _id: 'w1', isVerified: true, verificationStatus: 'approved', user: { name: 'Ali' } };
    Worker.findByIdAndUpdate.mockReturnValue({
...
describe('rejectWorker', () => {
  ...
  it('rejects worker and sets status to rejected', async () => {
    const worker = { _id: 'w1', verificationStatus: 'rejected', user: { name: 'Ali' } };
    Worker.findByIdAndUpdate.mockReturnValue({
      populate: jest.fn().mockResolvedValue(worker),
    });
